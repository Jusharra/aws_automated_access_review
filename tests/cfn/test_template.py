...
def test_template_has_required_resources():
    ...
    assert re.search(r"AccessReviewLambda:\s+Type:\s+AWS::Lambda::Function", content, re.MULTILINE), "Lambda function resource is missing"
    assert re.search(r"ReportBucket:\s+Type:\s+AWS::S3::Bucket", content, re.MULTILINE), "S3 bucket resource is missing"
    assert re.search(r"AccessReviewLambdaRole:\s+Type:\s+AWS::IAM::Role", content, re.MULTILINE), "Lambda execution role is missing"
...
def test_lambda_has_required_properties():
    ...
    lambda_section = re.search(r"AccessReviewLambda:.*?(?=\n\w+:|$)", content, re.DOTALL)
    assert lambda_section, "Lambda resource not found"
    
    lambda_content = lambda_section.group(0)
    assert "Type: AWS::Lambda::Function" in lambda_content, "Incorrect Lambda resource type"
...
def test_s3_bucket_has_required_properties():
    ...
    assert "Type: AWS::S3::Bucket" in bucket_content, "Incorrect S3 bucket resource type"
...
def test_lambda_role_has_required_policies():
    ...
    role_section = re.search(r"AccessReviewLambdaRole:.*?(?=\n\w+:|$)", content, re.DOTALL)
    ...
    assert "AssumeRolePolicyDocument:" in role_content, "Missing AssumeRolePolicyDocument"
    assert "ManagedPolicyArns:" in role_content or "Policies:" in role_content, "Missing policies"
